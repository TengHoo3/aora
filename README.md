...
- **`pipeline.py`** - baseline template with safety guardrails baked in. Agents **copy** this into a task directory and experiment there. Never modified in the repo root.
...
Each experiment lives in its own task directory (`tasks/<taskname>_<datetime>/`). The agent copies `pipeline.py` and `train.py` from the repo root, edits within the task directory, runs, checks if the score improved, keeps or discards, and repeats — exactly like the LLM pretraining version but for tabular ML.
...
```mermaid
flowchart TD
    A["Read eda_report.md"] --> B["Edit tasks/.../pipeline.py\n(sklearn/XGBoost/LightGBM)"]
    B --> C["Pre-flight cost check\n(_estimate_search_seconds)"]
    C --> D["uv run tasks/.../pipeline.py > run.log"]
    D --> E["Overfitting check\n(train score vs CV mean)"]
    E --> F{Score improved?\noverfit_gap < 0.05?}
    F -- Yes --> G["Keep edit\nlog to results.tsv"]
    F -- No --> H["Revert pipeline.py\nlog discard/crash"]
    G --> B
    H --> B
    B --> I{Exhausted?\nBelow threshold?}
    I -- Yes --> J["Edit tasks/.../train.py\nTabularMLP fallback"]
    J --> K["curl host_bridge /run_train"]
    K --> F
    I -- No --> B
```
...
# 5. Check the EDA report (generated in repo root)
cat eda_report.md

# 6. Create a task directory and copy the templates
mkdir -p tasks/mytask_20260408_120000
cp pipeline.py train.py tasks/mytask_20260408_120000/
cp eda_report.md tasks/mytask_20260408_120000/

# 7. Run the baseline experiment (always from repo root)
uv run tasks/mytask_20260408_120000/pipeline.py
```
...
Each run also outputs `cv_mean`, `train_score`, and `overfit_gap` for diagnosing overfitting. A gap > 0.05 means the result should be discarded regardless of the test score.
...
- **Task directories isolate experiments.** Each run gets its own `tasks/<name>_<datetime>/` directory. The repo root `pipeline.py` and `train.py` are templates only — never modified.
- **Safety guardrails in the template.** `pipeline.py` ships with `_size_caps()`, `_estimate_search_seconds()`, and `_overfitting_check()` so agents avoid runaway searches and catch overfitting before logging results.
...
- **Time budget.** Each experiment is bounded by `TIME_BUDGET = 300` seconds. Both `pipeline.py` (via pre-flight cost check + `RandomizedSearchCV` `n_iter` limits) and `train.py` (via early stopping + time guard) respect this.
