...
Spin up your Claude or Codex agent in this repo and prompt:
...
pipeline.py       — Tier 1: sklearn/XGBoost/LightGBM experiments (agent edits)
train.py          — Tier 2: PyTorch TabularMLP neural fallback (agent edits if escalated)
program.md        — agent instructions (human edits)
pyproject.toml    — dependencies (includes torch)
data/             — place your dataset here
eda_report.md     — generated by uv run prepare.py
results.tsv       — experiment log (created by agent)
run.log           — last run output (created by agent)
```
